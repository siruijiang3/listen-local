...
**0.1.2 为预览版。** 支持原文与音频双向定位，已完成本机 GPU/CPU 推理与安装包构建；手机实机、全新 Windows 和全部性能验收仍有未完成项目，详见 [验证记录](docs/VALIDATION.md)。
...
点击「收听／继续收听」会进入「阅读与收听」页面。点击已生成的正文片段即可从该片段开头播放；拖动进度条会预览并定位对应原文，松手提交，暂停时跳转保持暂停。方向键每次调整五秒，PageUp/PageDown 调整三十秒，Home/End 定位开头/结尾，Escape 取消尚未提交的拖动。尚未生成的正文可以浏览，点击只提示状态，不改变生成顺序。

正文默认跟随音频；手动滚动、切换章节或正文部分后进入自由浏览，点击「回到正在朗读」恢复跟随。对应精度为**生成片段级**：高亮是当前片段的完整原文，不是逐字或逐句字幕。新任务以正文换行为段落边界，不跨段生成；长段仍拆成多个片段。已有音频无需重新生成，保留原分段；跨自然段的片段会显示明确说明及时间范围。
